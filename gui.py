...
class AppBlockerGUI:
    ...
    # === CHECKPOINT: Time Limit Delay Field ===
    def _create_time_limit_delay_field(self, settings_frame, start_row: int) -> int:
        """
        Render the delay field and normalize the configured delay hours.

        WHY: Keeps UI wiring and minimum-delay validation together so the
        configuration always stores a safe value before saving.
        """
        ...
        if self.config.get("time_limit_update_delay_hours") != delay_hours:
            self.config["time_limit_update_delay_hours"] = delay_hours

        self.delay_var = tk.StringVar(value=str(delay_hours))
        ttk.Label(
            settings_frame,
