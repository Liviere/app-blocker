...
class TestWithIsolatedUtils(unittest.TestCase):
    ...
    def test_isolated_config_context_manager(self):
        ...
        # Define test data
        test_apps = {"notepad.exe": 45, "paint.exe": 30}

        test_config = create_test_config(
            apps=test_apps, check_interval=15, enabled=True
        )

        test_log = create_test_log(
            {"2025-07-14": {"notepad.exe": 1200, "paint.exe": 800}}
        )
        ...
        with isolated_config(test_config, test_log) as manager:
            ...
            # Test log updates
            manager.update_log({"2025-07-15": {"calculator.exe": 600}})
